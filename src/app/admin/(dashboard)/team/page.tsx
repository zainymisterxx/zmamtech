  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold text-black dark:text-white">Team</h1>
          <p className="mt-2 text-gray-700 dark:text-gray-300">Manage your team members.</p>
        </div>
        <Link
          href="/admin/team/new"
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:border-neutral-800 bg-gray-100 dark:bg-neutral-800/50">
              <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Photo</th>
          ...
          <tbody>
            {!members || members.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-gray-700 dark:text-gray-300">
              ...
              members.map((member) => (
                ...
                >
                  <td className="px-6 py-4">
                    <div className="w-10 h-10 rounded-full overflow-hidden border border-gray-200 dark:border-neutral-800 bg-gray-100 dark:bg-neutral-800 flex items-center justify-center">
                      {member.image ? (
                        <img src={member.image} alt={member.name} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-sm font-bold text-brand-gold uppercase">{member.name?.charAt(0)}</span>
                      )}
                    </div>
                  </td>
                  <td className="px-6 py-4">
