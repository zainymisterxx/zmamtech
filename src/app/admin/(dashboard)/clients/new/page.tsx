...
export default function NewClientPage() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (logoFile) {
        logo_url = await uploadImage(logoFile, "clients")
      }

      const { error: insertError } = await supabase
        .from("clients")
        .insert([{ name, industry: industry || null, description: description || null, logo_url }])
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-fade-in">
      <div>
        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-2">
          <Link href="/admin/clients" className="hover:text-brand-gold transition">
            Clients
          </Link>
          <span>/</span>
          <span className="text-gray-700 dark:text-gray-300 font-medium">New Client</span>
        </div>
        <h1 className="font-heading text-3xl font-bold text-black dark:text-white">Add Client</h1>
      </div>
      ...
      <div className="bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800 rounded-xl p-8 shadow-soft">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex flex-col gap-3">
            <label className="text-sm font-medium text-text-secondary">Logo (optional)</label>
            {logoPreview && (
